...
public class DriverServiceImple implements DriverService {
    ...
    @Override
    public void saveDriver(DriverDTO driverDTO){ driverRepo.save(mapper.map(driverDTO, Driver.class));}
    @Override
    public DriverDTO getDriver(String id){ return mapper.map(driverRepo.findAllById(id),DriverDTO.class);}
    @Override
